...
After feature selection, the results of feature importance from the top three (3) models are fed to a Generative AI (GenAI) model for interpretation. That part of the code containing API key is later removed due to privacy issues.


**6.	Model Evaluation**
   
The GenAI model provided the following feature importance analysis on the results of the top three (3) classification models.  There are slight changes between runs and theyt usually come from randomness in the pipeline but the ranking is generally maintained.

**Decision Tree**
`csat_score` (0.323722): This is the most significant feature in predicting customer churn. A high `csat_score` is associated with lower churn rates, suggesting that customers who are satisfied with the service are less likely to leave. 

`tenure_months` (0.278752): Long-term customers are less likely to churn, indicating that tenure is a strong predictor of customer loyalty. 

`monthly_logins` (0.198848): Customers who actively use the service are more likely to remain as customers, whereas inactive customers may be more prone to churning.

Recommendations: 
1. Monitor `csat_score`: Regularly track scores to identify potential issues with service quality or satisfaction. 
2. Focus on long-term customers: Develop strategies to retain long-term customers and prevent them from churning. 
3. Address payment issues: Investigate payment failures and resolve any issues related to billing or transactions. 
4. Improve customer engagement: Increase customer engagement by offering incentives for regular use and referrals. 
5. Re-evaluate feature development: Consider removing or re-evaluating features that do not significantly impact customer churn.

**Random Forest**
`csat_score` (0.071467): This indicates that customer satisfaction is a strong predictor of churn. 

`tenure_months` (0.042717): Longer tenure (i.e., time with the company) increases the likelihood of churn. 

`monthly_logins` (0.025251): Customers who log in more frequently are less likely to churn, suggesting that regular usage is a crucial factor in customer retention.

`payment_failures` (0.021220): Payment-related issues can lead to churn, highlighting the importance of reliable payment processes. 

Recommendations: 
1. Focus on improving customer satisfaction (`csat_score`) to reduce churn. 
2. Analyze payment-related issues (`payment_failures`) and implement measures to prevent failures. 
3. Engage with customers through regular interactions (`monthly_logins`). 
4. Review pricing models (`tenure_fee_interaction`) to ensure they do not negatively impact customer retention. 
5. Develop targeted marketing strategies for male customers and SMEs. 

This analysis provides a starting point for understanding the most influential factors driving customer churn. Further investigation and modeling can refine the insights and inform data-driven decisions to improve customer retention.

**XG Boost**
`csat_score` : The most important feature at 21.3% in predicting customer churn, which is a measure of customer satisfaction. This highlights the significance of ensuring customers are satisfied with the service. 

`tenure_months` : The second most important feature at 17.1%, which indicates that customers who have been with the service for a longer period are less likely to churn. This might be an opportunity to retain customers through loyalty programs or targeted promotions. 

`payment_failures` : A significant feature at 11.5% in predicting churn, suggesting that customers who have experienced payment failures are more likely to leave the service. 

Recommendations: 
1. Prioritize improving `csat_score`  through service quality improvements or targeted customer satisfaction initiatives. 
2. Develop strategies to retain long-tenured customers through loyalty programs or personalized promotions. 
3. Investigate and resolve payment failures promptly to prevent churn. 
4. Consider revising pricing strategies to focus on other factors that affect churn. 

These insights should guide further exploration and potential interventions to mitigate customer churn.

**Overall Model Summary**     

Based on the metrics of Accuracy, Precision, Recall, F2 Score, AUC and Profit/Loss, the `Random Forest Classifier` and `XGBoost Classifier` are virtually tied for the top spot (the slight lead of `Random Forest Classifier` could be within the statistical margin of error) while the `Decision Tree Classifier` is not very far away (see comparison table below). 

For this particular dataset, either `XGBoost Classifier` or `Random Forest Classifier` is the recommended machine learning algorithm.
